...
import com.github.gobars.l2cache.aspect.config.CacheConfig;
...
public class CacheAspectTest {
  ...
  public void testEvictUserNoKey() {
    ...
    String key =
        "user:info:params:[300118,[w,四川,~！@#%……&*（）——+：“？》:''\\>?《~!@#$%^&*()_+\\\\],address:addredd:成都,age:122,birthday:1593530584170,height:18.2,income:22.22,lastName:[w,四川,~！@#%……&*（）——+：“？》:''\\>?《~!@#$%^&*()_+\\\\],lastNameList:[W,成都],lastNameSet:[成都,W],name:name,userId:300118]";
  ...
  public void testEvictAllUserNoCacheMannger() {
    ...
    sleep(2);
    ((L2Manager) cacheManager).getCacheContainer().clear();
    Assert.assertTrue(((L2Manager) cacheManager).getCacheContainer().size() == 0);
